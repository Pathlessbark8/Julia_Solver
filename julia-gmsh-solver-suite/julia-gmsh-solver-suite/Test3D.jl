...
# create matrices
# generic frequency independent matrices
@time M = massMatrix(global_space, coefficients=epsr_vector)
@time R = massMatrix(global_space, coefficients=sigma_vector)
@time S = curlMatrix(global_space)
@time Kabc = integrateRobinBoundary(global_space, 2001, 1.0)

breakpoint::Int32 = 1.5

# matrix of sources (not scaled by -jωμ0)
points, dir = createSources(24, ANTENNTA_RADIUS, CHAMBER_RADIUS)
B = zeros(N, size(points, 2))
for isource in axes(points, 2)
    p = points[:,isource]
    vector_integral, indexes = integratePointSource(global_space, p[1], p[2], p[3])
    projection = dir[:,isource]
    integral = dot.(vector_integral, Ref(projection))
    B[indexes, isource] = integral
end
...
freqs = LinRange(1e9, 2.4e9, 1)
collected_data = []
for f in freqs
    println("Starting solution for f=$f")
    ω = 2*pi*f
    Y0 = 1im*ω*sqrt(ϵ0*μ0)
    K = S - (ω^2 * ϵ0 * μ0)*M + (1im*ω*μ0)*R + Y0*Kabc

    # find solution
    Bf = (-1im*ω*μ0) .* B[non_pec_list, :]
    ...
    samples = Ms * Uf
    push!(collected_data, reshape(samples, (1, size(samples)...)))

    # U = Vector{ComplexF64}[]
    # for isource in axes(points, 2)
    #     temp = zeros(ComplexF64, N)
    #     temp[non_pec_list] = Uf[:,isource]
    #     push!(U, temp)
    # end
...
matrix = reduce(vcat, collected_data)

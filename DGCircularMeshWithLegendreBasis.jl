...
numComponents, basisFunctions, numOrientations = gmsh.model.mesh.getBasisFunctions(elementType1D, localCoords, "H1Legendre$setMeshOrder")
println("Number of components are " * string(numComponents))
basisFunctionOrientation = gmsh.model.mesh.getBasisFunctionsOrientation(elementType1D, "H1Legendre$setMeshOrder")
println("Basis function orientation is " * string(basisFunctionOrientation))
println("Number of orientations are " * string(numOrientations))
pp("basis functions at integration points", basisFunctions, 1)
print(length(basisFunctions))


# basisFunctions = reshape(basisFunctions, (2*(setMeshOrder+1),length(basisFunctions) ÷ (2*(setMeshOrder+1))))
basisFunctions = basisFunctions[1:length(basisFunctions) ÷ 2]
basisFunctions = reshape(basisFunctions, ((setMeshOrder+1),length(basisFunctions) ÷ ((setMeshOrder+1))))



pp("basis functions at integration points", basisFunctions, 1)
println("Length of x is " * string(length(x)))
println("Length of basisFunctions is " * string(length(basisFunctions)))
...
f
sc = display(f);
save("LegendreBasiswithOrder$setMeshOrder.png",f)
